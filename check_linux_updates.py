...
def get_update_line(host, updates, sec_updates, reboot_required):
    ...
    return ((u'{:<%d}: updates: {:>6}, reboot-required: {}' % env.host_column_size)
...
def do_check_updates():
    ...
    if result.succeeded:
        ...
        if not exists('/usr/lib/update-notifier/apt-check'):
            print((u'{:<%d}: (apt-check is not available)' % env.host_column_size)
                  .format(env.host))
            return
        ...
        if result.failed:
            ...
            return
        (updates, sec_updates) = map(lambda x: int(x), str(result.stdout).split(';'))
        ...
        # yum returns 0 when there's no update and returns 100 when there are update(s).
