...
class TesterBase(object):
    ...
    def get_instance(cls, args):
        ...
        if os.path.exists(DebianTester.DEBIAN_VERSION_FILE):
            logger.debug('{} exists. Assuming debian-like system.'
                         .format(DebianTester.DEBIAN_VERSION_FILE))
            if logger.level <= DEBUG:
                f = open(DebianTester.DEBIAN_VERSION_FILE, 'r')
                logger.debug('debian-version: {}'.format(f.read().rstrip()))
            ...
            if not os.path.exists(DebianTester.APT_CHECK_FILE):
                logger.error('{} does not exist while {} exists.'
        ...
        if os.path.exists(RedhatTester.REDHAT_RELEASE_FILE):
            logger.debug('{} exists. Assuming redhat-like system.'
                         .format(RedhatTester.REDHAT_RELEASE_FILE))
            if logger.level <= DEBUG:
                f = open(RedhatTester.REDHAT_RELEASE_FILE)
                logger.debug('redhat-release: {}'.format(f.read().rstrip()))
...
class DebianTester(TesterBase):
    ...
    def get_update_count(self, is_security_updates=False):
        ...
        p.wait()
        if (p.returncode > 0):
            raise RuntimeError('apt-check failed with {}. err:\n{}'
...
class RedhatTester(TesterBase):
    ...
    def needs_reboot(self):
        ...
        p1.wait()
        if p1.returncode:
            raise RuntimeError('Failed to run "{}" (ret: {}). stderr:\n{}'
        ...
        logger.debug('latest_kernel_line: {}'.format(latest_kernel_line))
        cmd2 = 'uname -r'
        ...
        logger.debug('current_kernel: {}'.format(current_kernel))
